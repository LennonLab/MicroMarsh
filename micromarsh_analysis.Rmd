...
design <- inner_join(data_frame(sample_ID = rownames(dna)), 
  ...
  inner_join(design) %>% filter(treatment %in% c("F", "PU", "PR"))
...
as.data.frame(scores(dna.pcoa)) %>% 
  ...
  ggsave("figures/ordination-dna.png", width = 8, height = 6, units = "in", dpi = 500)
...
as.data.frame(scores(rna.pcoa)) %>% 
  ...
  labs(x = paste0("PCoA 1 (", explained[1],"%)"),
       y = paste0("PCoA 2 (", explained[2],"%)")) + 
  ggsave("figures/ordination-rna.png", width = 8, height = 6, units = "in", dpi = 500)
