...
design <- inner_join(tibble(sample_ID = rownames(otus)), design) %>% 
  ...
  filter(month(date, label = T) %in% c("Oct", "Jul")) %>% 
  filter(year(date) %in% c(2015, 2016)) # subset just to sequential fall samples
...
data.frame(richness = otus.rich, diversity = otus.alpha) %>% rownames_to_column("sample_ID") %>% 
  ...
  ggplot(aes(x = treatment, y = diversity, fill = molecule)) +
  # geom_point(alpha = 0.5) +
  geom_boxplot(position = position_dodge(), alpha = .7) +
  facet_grid(~ date) +
  scale_fill_grey() +
  labs(x = "", y = "Alpha diversity (species equivalents, D = 1)") +
  ggsave("figures/alpha.png", width = 6, height = 4, units = "in", dpi = 500)
...
as.data.frame(rowSums(otus.rel[,desulf.cols])) %>% 
  ...
  #scale_y_log10() +
  facet_grid(molecule ~ date) + 
  labs(y = "Percent potential sulfate reducers", x = "") +
  ggsave("figures/sulfate-reduction.png", width = 5, height = 5, units = "in", dpi = 500)

env %>% 
  ggplot(aes(y = Salinity, x = treatment)) + 
  geom_point()
...
```{r}
env_vars <- env %>% select(molecule, DRP, NH4, NO2_3, DOC, Sulfides, Cl, Salinity, Soil_surface_temp)
...
as.data.frame(scores(rda.out)$sites) %>% 
  ...
  geom_segment(data = env.vecs, size = .5,
               ...
               arrow = arrow(angle = 20, 
                             ...
                             type = "open")) +
  geom_label_repel(data = env.vecs, size = 3,
                  aes(x = (scale.vecs-.2)*RDA1,
                      y = (scale.vecs-.2)*RDA2,
                      label = rownames(env.vecs)),
                  color = "black", segment.alpha = 0) +
  stat_ellipse() +
  ggsave("figures/rda.png", width = 10, height = 6, units = "in", dpi = 500)
