...
rownames_to_column(data.frame(otu.rich, d1 = exp(diversity(otus, "shannon"))), "sample_ID") %>% 
  ...
  scale_fill_viridis(begin = .2, end = .8, discrete = T) +
...
design
perma <- adonis(otus ~ treatment * as.factor(date), method = "euclidean", data = design)
perma
```

```{r}
dist.full <- vegdist(otus.full, method = "euclidean")
design.full
```