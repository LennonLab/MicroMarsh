...
pairwise.perm <- pairwise.perm.manova(resp = vegdist(otus.hel, method = "euclidean"), fact = design$treatment, nperm = 9999, R2 = TRUE, F = TRUE)
...
# anova(rda.out)
# output:
# Model: rda(formula = otus.hel ~ DRP + NH4 + NO2_3 + Sulfides + Salinity + Soil_surface_temp, data = as.data.frame(scale(env_vars)))
#          Df Variance      F Pr(>F)   
...
fit <- envfit(rda.out, env_vars, permutations = 999, display = "lc")
fit
#plot(rda.out, type = "p")
#plot(fit, col = "red")
vif.cca(rda.out)
...
plot_grid(sulf.plot, methan.plot, labels = c("a.", "b."), label_fontface = "plain") +
  ggsave("figures/funtional_response.png", width = 8, height = 3.2, units = "in", dpi = 500)
